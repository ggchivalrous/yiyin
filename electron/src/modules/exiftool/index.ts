import type { Exif } from './interface'
  ...
  private formatExiftoolParseInfo(record: Record<string, string>): Exif {
    ...
    if (exif.MeteringMode) {
      switch (exif.MeteringMode.toLowerCase()) {
        ...
        case 'multi-segment':
        case 'multi-zone':
          exif.MeteringMode = '评价测光'
          break
        case 'spot':
          exif.MeteringMode = '点测光'
          break
        case 'partial':
          exif.MeteringMode = '局部测光'
          break
        case 'average':
          exif.MeteringMode = '平均测光'
          break
        case 'center-weighted':
          exif.MeteringMode = '中央重点测光'
          break
        case 'highlight-weighted ':
          exif.MeteringMode = '斑马测光'
          break
        default: exif.MeteringMode = ''
          break
      }
    }
  ...
  private formatExifParserInfo(record: Record<string, string>): Exif {
    ...
    if (exif.ExposureProgram) {
      switch (+exif.ExposureProgram) {
        case 0: exif.ExposureProgram = 'Auto'
          break
        case 1: exif.ExposureProgram = 'M'
          break
        case 2: exif.ExposureProgram = 'P'
          break
        case 3: exif.ExposureProgram = 'A'
          break
        case 4: exif.ExposureProgram = 'S'
          break
        default: break
      }
    }
    ...
    if (exif.WhiteBalance) {
      switch (+exif.WhiteBalance) {
        case 0: exif.WhiteBalance = 'Auto'
          break
        case 1: exif.WhiteBalance = '手动'
          break
        default: break
      }
    }
