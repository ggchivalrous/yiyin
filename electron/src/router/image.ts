...
r.listen<any, boolean>(routerConfig.startTask, async (data: StartTaskData) => {
  ...
  for (const fileInfo of data.fileUrlList) {
    ...
    // 生成背景图片
    const blurInfo = await imgTool.createBgImg(path.resolve(`${imgPath}_bg.jpg`)).catch((e) => console.log('模糊图片生成失败', e));

    // 生成主图
    const scaleInfo = await imgTool.createMainImg(path.resolve(`${imgPath}_main.jpg`)).catch((e) => console.log('缩放图片生成失败', e));
